...
`drillshaft` is my package and config manager that allows me to set up
...
source with `cargo install drillshaft --git https://github.com/Pistonite/drillshaft --locked`.
When upgrading with the `drillshaft` command, it will download from github releases
...
1. Setup `DRILLSHAFT_HOME`. This will let you pick a location for `drillshaft`
   to store everything. The default for Windows is `%USERPROFILE%/.config/pistonite-drillshaft`.
   If you use a dev drive, set the path somewhere on the dev drive, like `X:/.config/pistonite-drillshaft`
   where `X` is the drive label for the dev drive.
1. The setup will add `DRILLSHAFT_HOME` to the current user's environment variables.
   It will also modify the current user's `PATH` to include everything installed by `drillshaft`.
1. Add the `drillshaft` init script to Windows PowerShell (AKA PowerShell 5)'s `$PROFILE.CurrentUserAllHosts`.
   ```powershell
   # Drillshaft init script
   . $env:DRILLSHAFT_HOME\init.pwsh
   ```
   Note that PowerShell 7 is not included in Windows. It's installed with the `windows-pwsh` package in `drillshaft`.
   `drillshaft` will manage all configurations for PowerShell 7.
1. Restart Windows PowerShell.

### Non-Windows
1. Run the downloaded/installed binary in a terminal.
1. Setup `DRILLSHAFT_HOME`. This will let you pick a location for `drillshaft`
   to store everything. The default for Non-Windows is `~/.config/pistonite-drillshaft`.
1. Modify your shell profile (for example `~/.bashrc`) to source the following file:
   ```bash
   # Drillshaft init script
   . ~/.config/pistonite-drillshaft/init.bash 
   ```
   Replace the path with the correct path to the installation. The init script
   will set up the `DRILLSHAFT_HOME` and `PATH` environment variables.
   You have to use the full path when you source the file.
1. Restart the terminal.

## Upgrade
`drillshaft` will track the compatible versions of installed software. This "registry"
...
| `ds upgrade` | Upgrade the `drillshaft` binary and registry |
