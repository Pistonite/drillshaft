use std::{ffi::OsStr, mem::MaybeUninit, ops::{Deref, DerefMut}, time::Duration};
...
pub fn ensure_terminated(process_name: &str) -> cu::Result<()> {
    ...
    if s.processes_by_exact_name(process_name.as_ref()).next().is_none() {
        return Ok(());
    }
    for _ in 0..5 {
    ...
    cu::bail!("process '{process_name}' did not terminate - please retry after stopping the process manually");
}
