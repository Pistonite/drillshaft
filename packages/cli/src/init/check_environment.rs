    ...
    pub fn check_init_environment(config: &Config) -> cu::Result<()> {
        if config.windows.control_personal_shell_folder {
            ...
            if value != expected_value {
                cu::warn!(
                    "the current user shell folder is: '{value}', which is not managed by shaft"
                );
                cu::hint!("  this is the location that stores powershell user profiles");
                cu::hint!(
                    "  set it to {expected_value_str} can reduce clutter in your other folders"
                );
                if cu::yesno!("change the user shell folder to {expected_value_str}?")? {
                    ...
                    if !opfs::is_sudo() {
                        cu::bailfyi!(
                            "setting registry requires sudo - please run `sudo shaft -vV`"
                        );
                    }
                    let reg_key = cu::check!(
                    ...
                    cu::bailfyi!(
                        "please check and copy over your current shell profiles if needed, then restart all terminal processes to reload the environment"
                    );
                } else {
